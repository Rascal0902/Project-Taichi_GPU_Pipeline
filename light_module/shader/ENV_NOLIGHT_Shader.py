...
class Environment_Shader:
    ...
    def fragment_shader(self, normal, obj_color, viewdir, frag_pos):
        ...
        reflectDir = ti.math.reflect(view, norm)
        reflectDir = ti.math.normalize(reflectDir)

        ceta = ti.math.acos(reflectDir[2])
        phi = ti.math.atan2(-reflectDir[1], reflectDir[0])
        ...
        obj_color = obj_color / 255

        return ti.Vector([obj_color[0] * (env_color[0] + self.N - 1)/self.N, obj_color[1] * (env_color[1] + self.N - 1)/self.N, obj_color[2] * (env_color[2] + self.N - 1)/self.N])

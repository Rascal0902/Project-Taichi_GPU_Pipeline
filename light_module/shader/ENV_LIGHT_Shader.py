...
@ti.data_oriented
class Environment_Light_Shader:
    def __init__(self, mesh, space, img, env, N=1, BACKFACE=True):
    ...
    def rasterizer(self, cam_posx:ti.f32, cam_posy:ti.f32, cam_posz:ti.f32):
        for index in range(self.ti_face.shape[0]):
            ...
            if screen_v1[2] < 0 or screen_v2[2] < 0 or screen_v3[2] < 0:
                continue
            ...
            for x, y in ti.ndrange((min_x - 1, max_x + 1), (min_y - 1, max_y + 1)):
                ...
                if w0 >= 0.0 and w1 >= 0.0 and w2 >= 0.0:
                    ...
                    color = self.img.pixel[int(tex_pos[0] * self.img.image_width), int(tex_pos[1] * self.img.image_height)][:3]
    ...
    def fragment_shader(self, normal, obj_color, viewdir, frag_pos):
        ...
        reflectDir_cam = ti.math.reflect(view, norm)
        reflectDir_cam = ti.math.normalize(reflectDir_cam)

        ceta = ti.math.acos(reflectDir_cam[2])
        phi = ti.math.atan2(-reflectDir_cam[1], reflectDir_cam[0])

        u = (phi + ti.math.pi)/(2 * ti.math.pi)
        v = ceta / ti.math.pi

        env_color = self.env.pixel[int(v * self.env.image_width), int(u * self.env.image_height)][:3]
        env_color = env_color / 255

        obj_color = ti.Vector([obj_color[0] * (env_color[0] + self.N - 1)/self.N, obj_color[1] * (env_color[1] + self.N - 1)/self.N, obj_color[2] * (env_color[2] + self.N - 1)/self.N])

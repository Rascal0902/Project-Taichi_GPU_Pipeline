...
class PLY_Shader:
    def __init__(self, mesh, space, img, BACKFACE=True):
        self.ti_face = ti.field(dtype=ti.i32, shape=(mesh.obj['faces'].shape[0], 3))
        self.ti_color = ti.Vector.field(3, dtype=ti.f32, shape=(mesh.obj['colors'].shape[0]))
    ...
    def vertexShader(self, object_to_world_Matrix, world_to_view_Matrix, view_to_clip_Matrix):
        ...
        self.ti_color.from_torch(self.mesh.obj["colors"])
    ...
    def rasterizer(self, cam_posx:ti.f32, cam_posy:ti.f32, cam_posz:ti.f32):
        for index in range(self.ti_face.shape[0]):
            ...
            v1 = self.ndc[int(self.ti_face[index, 0])]
            v2 = self.ndc[int(self.ti_face[index, 1])]
            v3 = self.ndc[int(self.ti_face[index, 2])]
            ...
            for x, y in ti.ndrange((min_x - 1, max_x + 1), (min_y - 1, max_y + 1)):
                ...
                if w0 >= 0.0 and w1 >= 0.0 and w2 >= 0.0:
                    ...
                    fragment_pos = (w0 * self.world[int(self.ti_face[index, 0])] + w1 * self.world[
                        int(self.ti_face[index, 1])]
                                    + w2 * self.world[int(self.ti_face[index, 2])])

                    # get texture color
                    tex_color = (w0 * self.ti_color[int(self.ti_face[index, 0])] + w1 * self.ti_color[
                        int(self.ti_face[index, 1])]
                               + w2 * self.ti_color[int(self.ti_face[index, 2])])

                    normal = (w0 * self.normal[int(self.ti_face[index, 0])] + w1 * self.normal[
                        int(self.ti_face[index, 1])]
                              + w2 * self.normal[int(self.ti_face[index, 2])])

                    view_dir = ti.Vector([cam_posx - fragment_pos[0], cam_posy - fragment_pos[1], cam_posz - fragment_pos[2]]).normalized()

                    frag_color = self.fragment_shader(normal, tex_color, view_dir, fragment_pos)
